...
class FresnoyProfile(models.Model):
    ...
    deathplace_country = CountryField(null=True, default="", blank=True)
    ...
    def clean(self):
        ...
        if self.user is None and self.nickname == "":
            raise ValidationError(
                "No user is defined, set the nickname if you want to create an artist collective"
            )
    ...
    def __str__(self):
        if self.user:
            return (
                ...
                if self.nickname
                else "{} {}".format(self.user.first_name, self.user.last_name)
            )
        if self.nickname != "":
...
class Staff(models.Model):
    ...
    def __str__(self):
        ...
        if (
            self.user.artist_set.count() > 0
            and self.user.artist_set.all().first().nickname != ""
        ):
            return "{} ({})".format(
                self.user.artist_set.all().first().__str__(), self.user
            )
