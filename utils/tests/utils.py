...
def validate_jwt_token(token, user):
    ...
    return (
        infos
        and "user_id" in infos
        and User.objects.filter(pk=infos["user_id"]).exists()
    )
...
def parametrize_user_roles(metafunc):
    ...
    if (
        ...
        if None in metafunc.cls._user_roles:
            ...
            parametrization = [(None, None)] + list(
                zip(
        ...
        hasattr(metafunc.cls, "_auth_methods")
        and "auth_method" in metafunc.fixturenames
    ):
...
class AbstractHelpTestForAPI:
    ...
    methods_behavior = {
        method: 200 for method in ["list", "get", "patch", "put", "post", "delete"]
    }
    ...
    def thats_all_folk(self, method, response, user_role):
        if method in self.methods_behavior:
            ...
            print(
                "{} -- test code : {} / expected code : {}".format(
                    method, response.status_code, expected
                )
            )
    ...
    def test_get(self, client, user_role, auth_method, request):
        ...
        kwargs = self.prepare_request(client, user_role, auth_method)
        response = client.get(
            "{}/{}".format(self.base_url, self.target_uri_suffix()), **kwargs
        )
    ...
    def test_post(self, client, user_role, auth_method, request):
        ...
        fields = getattr(
            self, "post_fields", getattr(self, "put_fields", self.mutate_fields)
        )
